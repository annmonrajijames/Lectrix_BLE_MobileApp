...
uint8_t notify_data[400] = {
    ...
    0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20
        ...
        if (global_gatts_if != ESP_GATT_IF_NONE) {
            for (int i = 0; i < sizeof(notify_data); i += 20) {
                int chunk_size = (i + 20 < sizeof(notify_data)) ? 20 : (sizeof(notify_data) - i);
                esp_ble_gatts_send_indicate(global_gatts_if, gl_profile_tab[PROFILE_A_APP_ID].conn_id,
                                             gl_profile_tab[PROFILE_A_APP_ID].char_handle,
                                             chunk_size, &notify_data[i], false);
                vTaskDelay(pdMS_TO_TICKS(25)); // Add a small delay to avoid overwhelming the stack
                count=count+1;
                printf("count = %d", count);
            }
        }

        vTaskDelay(pdMS_TO_TICKS(25)); // Adjust timing as needed
    }
