...
#define TEST_DEVICE_NAME            "Srijananiiiiiiiiiiiii" 
...
// Declare a static variable to store the received value
static uint16_t soc_value = 0;  // Use the appropriate data type
...
typedef struct {
    uint8_t *prepare_buf; // Buffer for prepared write data
...
void example_exec_write_event_env(prepare_type_env_t *env, esp_ble_gatts_cb_param_t *param) {
    if (param->exec_write.exec_write_flag == ESP_GATT_PREP_WRITE_EXEC) {
        ESP_LOGI(GATTS_TAG, "Executing prepared write");
    } else {
        ESP_LOGI(GATTS_TAG, "Cancelling prepared write");
    }

    // Free the prepare buffer
    if (env->prepare_buf) {
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ESP_LOGI(GATTS_TAG, "ESP_GATTS_WRITE_EVT triggered");

        if (!param->write.is_prep) {
            uint8_t *received_data = param->write.value;
            uint16_t data_len = param->write.len;

            ESP_LOGI(GATTS_TAG, "Received data (length = %d):", data_len);
            esp_log_buffer_hex(GATTS_TAG, received_data, data_len);

            // Check if the data length is sufficient for all components (CAN ID, byte value, bit number, and command)
            if (data_len >= 4) {
                // Extract the CAN ID (assuming it's 2 bytes for this example)
                uint16_t can_id = (received_data[0] << 8) | received_data[1];

                // Extract the byte value (1 byte)
                uint8_t byte_value = received_data[2];

                // Extract the bit number (1 byte, between 0-7)
                uint8_t bit_number = received_data[3] & 0x07;  // Ensure it's between 0-7

                // Extract the command (assuming it's a single byte, you can adjust depending on the structure)
                char command = received_data[4];

                // Log the extracted data
                ESP_LOGI(GATTS_TAG, "CAN ID: 0x%04X", can_id);
                ESP_LOGI(GATTS_TAG, "Byte value: 0x%02X", byte_value);
                ESP_LOGI(GATTS_TAG, "Bit number: %d", bit_number);
                ESP_LOGI(GATTS_TAG, "Command: %c", command);  // Assuming the command is a single character

                // Optional: If you need to handle the extracted values further (e.g., using them for a command)
                // Example: You can process the command or bit number for specific actions
                // For example, you could use the byte_value and bit_number to check a specific bit:
                bool bit_status = (byte_value >> bit_number) & 0x01;
                ESP_LOGI(GATTS_TAG, "Bit %d status: %d", bit_number, bit_status);
            } else {
                ESP_LOGW(GATTS_TAG, "Unexpected data length, expected at least 4 bytes");
            }
        }
    ...
    case ESP_GATTS_EXEC_WRITE_EVT: {
        ESP_LOGI(GATTS_TAG, "ESP_GATTS_EXEC_WRITE_EVT triggered");
        ...
        if (param->exec_write.exec_write_flag == ESP_GATT_PREP_WRITE_EXEC) {
            ...
            if (a_prepare_write_env.prepare_buf) {
                ...
                if (a_prepare_write_env.prepare_len == sizeof(uint16_t)) {
                    uint16_t received_value = a_prepare_write_env.prepare_buf[0] | 
                                            (a_prepare_write_env.prepare_buf[1] << 8);
                    ESP_LOGI(GATTS_TAG, "Parsed 16-bit prepared value: %d", received_value);
                    soc_value = received_value;
                    ESP_LOGI(GATTS_TAG, "Updated SOC value from prepared write: %d", soc_value);
                }
            }
        } else {
