...
#define TEST_DEVICE_NAME            "Srijanani_Lectrix"
...
void transmit_task(uint8_t tenth_byte) {
    ...
    // Send Reset High to 0x18F60001 (Extended ID)
    twai_message_t transmit_message_reset_high = {
        .identifier = 0x18f60001,
        .data_length_code = 8,
        .extd = 1, // Extended frame
        .data = {Reset_bit_1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}
    };
    if (twai_transmit(&transmit_message_reset_high, 1000) == ESP_OK) {
        printf("Message sent to 0x18F60001 (High)\n");
    } else {
        ESP_LOGI(GATTS_TAG, "Failed to send Reset High to 0x18F60001");
    }

    vTaskDelay(pdMS_TO_TICKS(100));

    // Send Reset High to 0x144 (Standard ID)
    twai_message_t transmit_message_144_high = {
        .identifier = 0x144,
        .data_length_code = 8,
        .extd = 0, // Standard frame
        .data = {Reset_bit_1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}
    };
    if (twai_transmit(&transmit_message_144_high, 1000) == ESP_OK) {
        printf("Message sent to 0x144 (High)\n");
    } else {
        ESP_LOGI(GATTS_TAG, "Failed to send Reset High to 0x144");
    }

    vTaskDelay(pdMS_TO_TICKS(100));

    // Send Reset Low to 0x18F60001 (Extended ID)
    ...
    if (twai_transmit(&transmit_message_reset_low, 1000) == ESP_OK) {
        printf("Message sent to 0x18F60001 (Low)\n");
    } else {
        ESP_LOGI(GATTS_TAG, "Failed to send Reset Low to 0x18F60001");
    }

    vTaskDelay(pdMS_TO_TICKS(100));

    // Send Reset Low to 0x144 (Standard ID)
    twai_message_t transmit_message_144_low = {
        .identifier = 0x144,
        .data_length_code = 8,
        .extd = 0, // Standard frame
        .data = {Reset_bit_2, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}
    };
    if (twai_transmit(&transmit_message_144_low, 1000) == ESP_OK) {
        printf("Message sent to 0x144 (Low)\n");
    } else {
        ESP_LOGI(GATTS_TAG, "Failed to send Reset Low to 0x144");
    }
